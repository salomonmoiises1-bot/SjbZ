...
class EqualizerProcessor {
    ...
    var preampDb: Float = 0.0f
        set(value) { field = value.coerceIn(-12.0f, 12.0f) }
    ...
    fun setBandGain(index: Int, gainDb: Float) {
        if (index in 0 until BAND_COUNT) bandGains[index] = gainDb.coerceIn(-12.0f, 12.0f)
    }

    fun getBandGains(): FloatArray = bandGains.copyOf()

    fun setAllBands(gains: List<Float>) {
        for (i in 0 until minOf(gains.size, BAND_COUNT)) bandGains[i] = gains[i].coerceIn(-12.0f, 12.0f)
    }

    var bassBoostProcessor: BassBoostProcessor? = null

    // PARCHE: linkea el BassBoost y dispara re-apply al DSP
    fun linkBassBoost(bb: BassBoostProcessor, onChange: () -> Unit) {
        this.bassBoostProcessor = bb
        bb.onParametersChanged = { onChange() }
    }
    ...
    fun getEffectiveGain(index: Int, bassBoost: BassBoostProcessor? = null): Float {
        ...
        val bbBoost = if (bb!= null && bb.isEnabled) bb.getBoostGainForFrequency(ISO_FREQUENCIES[index]) else 0.0f
    ...
    fun applyPreset(presetName: String) {
        ...
        when (presetName) {
            "ATS-2835P Master" -> { preampDb = 0.0f; ats2835pProfileEnabled = true }
            "Bass Boost" -> {
            ...
            "Acoustic" -> {
                preampDb = -1.0f; ats2835pProfileEnabled = true
                val curve = floatArrayOf(
            ...
            "Flat" -> { preampDb = 0.0f; ats2835pProfileEnabled = false }
            "Rock" -> {
                preampDb = -1.5f; ats2835pProfileEnabled = true
                val curve = floatArrayOf(
            ...
            "Electronic" -> {
                preampDb = -2.0f; ats2835pProfileEnabled = true
                val curve = floatArrayOf(
