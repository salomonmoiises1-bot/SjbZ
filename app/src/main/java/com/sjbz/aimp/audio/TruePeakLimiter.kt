...
class TruePeakLimiter(
    sampleRate: Int,
    private val lookaheadMs: Float = 2.0f,
    private val releaseMs: Float = 80f,
    private val ceilingDb: Float = -1.0f
) {
    private var sr = sampleRate
    private var lookaheadSamples = ((sr * lookaheadMs) / 1000f).toInt().coerceAtLeast(32)
    private var delayBuf = FloatArray(lookaheadSamples * 8) // max 8ch
    ...
    private fun updateRelease() {
        releaseCoef = exp(-1f / (sr * releaseMs / 1000f))
    }
    ...
    // Zero-allocation: procesa in-place
    fun process(buf: FloatArray, totalFloats: Int, frames: Int, channels: Int) {
        ...
        for (f in 0 until frames) {
            ...
            val base = f * ch
            for (c in 0 until ch) {
                peak = max(peak, abs(buf[base + c]))
            }
            // gain objetivo
            val target = if (peak * currentGain > ceiling && peak > 1e-6f) {
                ceiling / peak
            } else 1f
            // release suave hacia 1.0, ataque instantáneo
            currentGain = if (target < currentGain) target
                          else currentGain + (1f - currentGain) * (1f - releaseCoef)
