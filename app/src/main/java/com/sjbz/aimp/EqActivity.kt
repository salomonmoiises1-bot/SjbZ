...
class EqActivity : AppCompatActivity() {
    ...
    private fun showGlobalHelpDialog() {
        AlertDialog.Builder(this).setTitle("Modo Global (Estilo Wavelet / Sin Root)")
    ...
    // FIX PRINCIPAL: evita que el ScrollView se mueva al ecualizar
    private fun blockParentScrollOnTouch(seekBar: SeekBar) {
        seekBar.setOnTouchListener { v, event ->
            when (event.action) {
                ...
                MotionEvent.ACTION_UP, MotionEvent.ACTION_CANCEL -> {
                    ...
                    var p = v.parent
                    while (p!= null) { p.requestDisallowInterceptTouchEvent(false); p = p.parent }
                }
            }
            false
        }
    ...
    private fun setup32BandSliders() {
        ...
        for (i in 0 until bandCount) {
            ...
            blockParentScrollOnTouch(seekBar)
            val bandIndex = i
    ...
    private fun setupPreampAndControls() {
        ...
        blockParentScrollOnTouch(seekBarPreamp); blockParentScrollOnTouch(seekBarSpeed); blockParentScrollOnTouch(seekBarCrossfade)
        seekBarPreamp.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(sb: SeekBar?, progress: Int, fromUser: Boolean) {
                ...
                if (fromUser &&!isUpdatingUiFromPreset) { equalizerProcessor.preampDb = db; PlaybackService.instance?.atsEngine?.updateEqualizer(); syncAllEffects() }
            }
            override fun onStartTrackingTouch(sb: SeekBar?) {}
            override fun onStopTrackingTouch(sb: SeekBar?) {}
        })
        seekBarSpeed.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(sb: SeekBar?, progress: Int, fromUser: Boolean) {
                ...
                if (fromUser) { PlaybackService.instance?.setPlaybackSpeed(factor); PlaybackService.instance?.audioChain?.setPlaybackParameters(factor, factor) }
            }
            override fun onStartTrackingTouch(sb: SeekBar?) {}
            override fun onStopTrackingTouch(sb: SeekBar?) {}
        })
        seekBarCrossfade.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            override fun onProgressChanged(sb: SeekBar?, progress: Int, fromUser: Boolean) {
                tvCrossfadeValue.text = "${progress}s"; if (fromUser) PlaybackService.instance?.atsEngine?.crossfadeSeconds = progress
            }
            override fun onStartTrackingTouch(sb: SeekBar?) {}
            override fun onStopTrackingTouch(sb: SeekBar?) {}
        })
    ...
    private fun setupMdrcGainControls() {
        ...
        for (i in 0 until 5) {
            ...
            blockParentScrollOnTouch(sb)
            sb.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
                override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
                    val gainDb = (progress - 120) / 10.0f; tv.text = String.format("%+.1f dB", gainDb)
                    if (fromUser &&!isUpdatingUiFromPreset) { mdrcProcessor.getBand(bandIndex)?.gainDb = gainDb; PlaybackService.instance?.atsEngine?.updateMDRC(); syncAllEffects() }
                }
        ...
        switchMdrcEnabled.setOnCheckedChangeListener { _, isChecked ->
            mdrcProcessor.isEnabled = isChecked; PlaybackService.instance?.atsEngine?.updateMDRC(); syncAllEffects()
    ...
    private fun refreshPresetsSpinner(selectPresetName: String? = null) {
        ...
        val target = selectPresetName?: presetManager.getActivePresetName(); val idx = names.indexOf(target)
    ...
    private fun setupButtons() {
        ...
        btnSavePreset.setOnLongClickListener { showColorPicker { chosenColor -> applyPresetColor(chosenColor); Toast.makeText(this, "Color de preset actualizado", Toast.LENGTH_SHORT).show() }; true }
        btnDeletePreset.setOnClickListener {
            ...
            AlertDialog.Builder(this).setTitle("Borrar Preset").setMessage("¿Deseas eliminar el preset '$selected'?").setPositiveButton("Borrar") { _, _ -> presetManager.deletePreset(selected); refreshPresetsSpinner("ATS-2835P Master"); Toast.makeText(this, "Preset eliminado", Toast.LENGTH_SHORT).show() }.setNegativeButton("Cancelar", null).show()
        }
        ...
        for (row in 0..1) {
            ...
            for (col in 0..4) {
                ...
                val btn = Button(this).apply {
                    ...
                    backgroundTintList = ColorStateList.valueOf(c); setOnClickListener { onColorSelected(c); dialog?.dismiss() }
                }
