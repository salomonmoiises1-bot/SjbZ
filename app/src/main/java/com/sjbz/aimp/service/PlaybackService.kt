...
class PlaybackService : MediaSessionService() {
    ...
    override fun onCreate() {
        ...
        com.sjbz.aimp.GlobalAudioSessionManager.getInstance(this).syncAudioEffects(atsEngine.equalizer, atsEngine)
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                CHANNEL_ID, "SjbZ Reproduccion de Audio",
                NotificationManager.IMPORTANCE_LOW
            ).apply {
                description = "Reproduccion continua de musica en segundo plano y pantalla bloqueada con DSP ATS2835P"
                setShowBadge(false)
