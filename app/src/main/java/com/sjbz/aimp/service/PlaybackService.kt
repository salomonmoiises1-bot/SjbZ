...
class PlaybackService : MediaSessionService() {
    ...
    override fun onCreate() {
        ...
        player.addListener(object : Player.Listener {
            override fun onPlaybackStateChanged(state: Int) {
                if (state == Player.STATE_ENDED) onSongEnded()
                handleWakeLockState(player.isPlaying)
    ...
    private fun createNotificationChannel(){
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.O){
            val ch = NotificationChannel(CHANNEL_ID,"SjbZ Reproducción",NotificationManager.IMPORTANCE_LOW).apply{ setShowBadge(false); lockscreenVisibility=Notification.VISIBILITY_PUBLIC }
            getSystemService(NotificationManager::class.java)?.createNotificationChannel(ch)
        }
    }
    private fun buildNotification(isPlaying: Boolean): Notification {
        val cur = getCurrentTrack()
        val title = cur?.title?: "SjbZ Hi-Res"
        val artist = cur?.artist?: "ATS-2835P DSP"
        ...
        return NotificationCompat.Builder(this,CHANNEL_ID)
           ...
           .setContentIntent(open).setOngoing(isPlaying).setOnlyAlertOnce(true).setVisibility(NotificationCompat.VISIBILITY_PUBLIC)
           .setPriority(NotificationCompat.PRIORITY_LOW).setCategory(NotificationCompat.CATEGORY_TRANSPORT)
           .addAction(android.R.drawable.ic_media_previous,"Ant",prev)
           .addAction(if(isPlaying) android.R.drawable.ic_media_pause else android.R.drawable.ic_media_play,if(isPlaying)"Pausa" else "Play",tog)
           .addAction(android.R.drawable.ic_media_next,"Sig",next)
           .addAction(android.R.drawable.ic_delete,"Stop",stop)
           .build()
    }
    private fun startForegroundWithNotification(p: Boolean){
        val n = buildNotification(p)
    ...
    fun playTrackAtIndex(i: Int, play: Boolean=true){
        ...
        if(play){ player.play(); audioChain.startFadeIn() }
        val sid=player.audioSessionId
        if(sid!=C.AUDIO_SESSION_ID_UNSET) audioChain.attachAudioSession(sid)
    ...
    override fun onDestroy(){
        ...
        mediaSession?.let{ val p=player; p.release(); it.release() }; mediaSession=null
        super.onDestroy()
    }
}
