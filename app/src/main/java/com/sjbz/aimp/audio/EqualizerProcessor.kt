...
class EqualizerProcessor {
    ...
    fun getBandGain(index: Int): Float = if (index in 0 until BAND_COUNT) bandGains[index] else 0.0f
    fun setBandGain(index: Int, gainDb: Float) { if (index in 0 until BAND_COUNT) bandGains[index] = gainDb.coerceIn(-12.0f, 12.0f) }
    fun getBandGains(): FloatArray = bandGains.copyOf()
    fun setAllBands(gains: List<Float>) { for (i in 0 until minOf(gains.size, BAND_COUNT)) bandGains[i] = gains[i].coerceIn(-12.0f, 12.0f) }
    ...
    fun applyPreset(presetName: String) {
        ...
        when (presetName) {
            ...
            else -> { preampDb = 0f; ats2835pProfileEnabled = true }
        }
    }

    // --- FIX CRITICO: AHORA ACEPTA bassBoostFreq y bassBoostGain ---
    fun toEqPreset(
        name: String,
        isCustom: Boolean = true,
        mdrcSettings: com.sjbz.aimp.model.MDRCSettings = com.sjbz.aimp.model.MDRCSettings(),
        color: Int = EqPreset.generateRandomColor(),
        bassBoostFreq: Int = 85,
        bassBoostGain: Float = 6f
    ): EqPreset {
        return EqPreset(
            name = name,
            preampDb = preampDb,
