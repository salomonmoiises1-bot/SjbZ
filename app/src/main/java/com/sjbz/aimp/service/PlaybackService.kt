...
class PlaybackService : MediaSessionService() {
    ...
    override fun onCreate() {
        ...
        player.addListener(object : Player.Listener {
            override fun onPlaybackStateChanged(playbackState: Int) {
                ...
                handleWakeLockState(isPlaying); updateNotification(isPlaying); onPlaybackStateChangedListener?.invoke(isPlaying)
            }
            ...
            override fun onMediaItemTransition(mediaItem: MediaItem?, reason: Int) { val currentTrack = getCurrentTrack(); updateNotification(player.isPlaying); onTrackChangedListener?.invoke(currentTrack, currentTrackIndex) }
        })
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(CHANNEL_ID, "SjbZ Reproducción de Audio", NotificationManager.IMPORTANCE_LOW).apply {
                description = "Reproducción continua con DSP ATS2835P"
    ...
    private fun buildNotification(isPlaying: Boolean): Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
           ...
           .setSubText(if (atsEngine.isBluetoothConnected) "ATS-2835P • Bluetooth A2DP" else "ATS-2835P • Hi-Res Direct")
            // PARCHE SjbZ - FIX CRASH NOTIFICACION - tu original tenia ic_launcher_foreground que crashea
    ...
    fun setPlaylist(tracks: List<Track>, startIndex: Int = 0, startPlaying: Boolean = true) {
        ...
        for (track in playlist) player.addMediaItem(MediaItem.Builder().setUri(track.uri).setMediaId(track.id.toString()).build())
        player.prepare()
