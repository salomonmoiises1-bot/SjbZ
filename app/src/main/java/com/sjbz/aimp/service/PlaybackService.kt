...
class PlaybackService : MediaSessionService() {
    ...
    override fun onCreate() {
        ...
        player.addListener(object : Player.Listener {
            override fun onPlaybackStateChanged(playbackState: Int) {
                ...
                handleWakeLockState(player.isPlaying); updateNotification(player.isPlaying); onPlaybackStateChangedListener?.invoke(player.isPlaying)
            }
            ...
            override fun onMediaItemTransition(mediaItem: MediaItem?, reason: Int) { updateNotification(player.isPlaying); onTrackChangedListener?.invoke(getCurrentTrack(), currentTrackIndex) }
        })
    ...
    private fun handleWakeLockState(isPlaying: Boolean) { try { if (isPlaying) { if (wakeLock?.isHeld == false) wakeLock?.acquire(2*60*60*1000L) } else { if (wakeLock?.isHeld == true) wakeLock?.release() } } catch (e: Exception) {} }
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(CHANNEL_ID, "SjbZ Reproducción", NotificationManager.IMPORTANCE_LOW).apply { setShowBadge(false); lockscreenVisibility = Notification.VISIBILITY_PUBLIC }
    ...
    private fun buildNotification(isPlaying: Boolean): Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID)
           ...
           .setSubText(if (atsEngine.isBluetoothConnected) "SB-Z • Bluetooth" else "SB-Z • ATS2835P Hi-Res")
           .setSmallIcon(R.drawable.ic_launcher_foreground).setContentIntent(openActivityIntent).setOngoing(isPlaying).setOnlyAlertOnce(true)
    ...
    private fun startForegroundWithNotification(isPlaying: Boolean) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) startForeground(NOTIFICATION_ID, n, ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PLAYBACK) else startForeground(NOTIFICATION_ID, n)
    }
    private fun updateNotification(isPlaying: Boolean) { try { (getSystemService(Context.NOTIFICATION_SERVICE) as? NotificationManager)?.notify(NOTIFICATION_ID, buildNotification(isPlaying)) } catch (e: Exception) {} }
    ...
    fun setPlaylist(tracks: List<Track>, startIndex: Int = 0, startPlaying: Boolean = true) {
        ...
        if (isSameList) { if (startIndex in playlist.indices) playTrackAtIndex(startIndex, startPlaying); return }
        playlist.clear(); playlist.addAll(tracks)
        val mediaItems = tracks.map { MediaItem.Builder().setUri(it.uri).setMediaId(it.id.toString()).build() }
    ...
    fun playTrackAtIndex(index: Int, startPlaying: Boolean = true) {
        ...
        savePlaybackSession(); updateNotification(startPlaying); onTrackChangedListener?.invoke(getCurrentTrack(), currentTrackIndex)
    }
    fun savePlaybackSession() {
        try {
            val prefs = getSharedPreferences("sbz_playback_session", Context.MODE_PRIVATE)
            prefs.edit().putLong("last_track_id", getCurrentTrack()?.id?: -1L).putInt("last_track_index", currentTrackIndex).putLong("last_position_ms", player.currentPosition).putFloat("last_speed", player.playbackParameters.speed).apply()
        } catch (e: Exception) {}
    }
    fun restorePlaybackSession(): Pair<Int, Long> { val p = getSharedPreferences("sbz_playback_session", Context.MODE_PRIVATE); return Pair(p.getInt("last_track_index", 0), p.getLong("last_position_ms", 0L)) }
