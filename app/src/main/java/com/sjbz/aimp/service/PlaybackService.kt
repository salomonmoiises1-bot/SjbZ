...
class PlaybackService : MediaSessionService() {
    ...
    private fun buildNotification(isPlaying: Boolean): Notification {
        ...
        return NotificationCompat.Builder(this, CHANNEL_ID).setContentTitle(title).setContentText(artist).setSubText(if (atsEngine.isBluetoothConnected) "ATS-2835P • Bluetooth A2DP" else "ATS-2835P • Hi-Res Direct").setSmallIcon(android.R.drawable.sym_def_app_icon).setContentIntent(openActivityIntent).setOngoing(isPlaying).setOnlyAlertOnce(true).setVisibility(NotificationCompat.VISIBILITY_PUBLIC).setPriority(NotificationCompat.PRIORITY_LOW).setCategory(NotificationCompat.CATEGORY_TRANSPORT).addAction(android.R.drawable.ic_media_previous, "Anterior", prevIntent).addAction(playPauseIcon, if (isPlaying) "Pausar" else "Reproducir", toggleIntent).addAction(android.R.drawable.ic_media_next, "Siguiente", nextIntent).addAction(android.R.drawable.ic_menu_close_clear_cancel, "Detener", stopIntent).build()
    }
    ...
    fun playNext() {
        ...
        if (nextIndex < playlist.size) playTrackAtIndex(nextIndex, true) else if (isLoopPlaylistEnabled) playTrackAtIndex(0, true)
    }
    fun playPrevious() {
        if (playlist.isEmpty()) return
        if (player.currentPosition > 3000) { player.seekTo(0); return }
        val prevIndex = currentTrackIndex - 1
        if (prevIndex >= 0) playTrackAtIndex(prevIndex, true) else if (isLoopPlaylistEnabled) playTrackAtIndex(playlist.size - 1, true)
    }
    ...
    fun getCurrentTrack(): Track? { return if (currentTrackIndex in playlist.indices) playlist[currentTrackIndex] else null }
