...
class SjbzDspProcessor : BaseAudioProcessor() {
    ...
    override fun onConfigure(inputAudioFormat: AudioProcessor.AudioFormat): AudioProcessor.AudioFormat {
        ...
        try { truePeakLimiter.setSampleRate(inputAudioFormat.sampleRate) } catch (_: Exception) {}
        isDirty = true
        return inputAudioFormat
    }
    override fun onReset() {
        s1.fill(0f); s2.fill(0f); emuS1.fill(0f); emuS2.fill(0f); emuEnv.fill(0f)
        fftRingBuffer.fill(0f); fftRingIndex = 0
        try { truePeakLimiter.reset() } catch (_: Exception) {}
        isDirty = true
    }
    override fun queueInput(inputBuffer: ByteBuffer) {
        ...
        for (frame in 0 until frames) {
            ...
            for (ch in 0 until channelCount) {
                var x = try { inputBuffer.float } catch (_: Exception) { 0f }
                x *= localLinearPreamp
                if (localMasterEnabled) {
                    ...
                    if (isEmuActive) {
                        ...
                        emuEnv[ch] = env
                        if (env > localThresh) wetX *= (localThresh / env).pow(0.75f)
            ...
            if (fftRingIndex >= FFT_BLOCK_SIZE) {
                fftRingIndex = 0
                localListener?.let { System.arraycopy(fftRingBuffer, 0, fftDispatchBuffer, 0, FFT_BLOCK_SIZE); it.invoke(fftDispatchBuffer) }
            }
        }
        outputBuffer.clear()
        for (i in 0 until totalFloats) { outputBuffer.putFloat(masteringScratch[i]) }
        outputBuffer.flip()
        try { inputBuffer.position(inputBuffer.limit()) } catch (_: Exception) {}
    }
    fun setPreamp(gainDb: Float) { val c = gainDb.coerceIn(-12f,12f); if (abs(preampDb-c)>0.001f){preampDb=c; linearPreamp=10.0.pow(c/20.0).toFloat()} }
