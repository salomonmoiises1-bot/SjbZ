...
/**
 * AudioChain: Coordinates the professional 32-bit floating-point audio processing chain:
 * Pipeline order: Input -> Bass (Low-Shelf RBJ) -> 32x Peaking EQ -> ATS2835P Emu -> Soft Clipper (tanh) -> Output
 *
 * Coefficients are recalculated only when sampleRate or DSP parameters change (dirty flag).
 * Coordinates playback parameters:
 * ExoPlayer -> SjbzDspProcessor -> Crossfade -> Stereo Balance / Pitch / Speed -> Stereo VU Meters
 */
class AudioChain(private val context: Context, val engine: ATS2835PEngine) {

    val dspProcessor: SjbzDspProcessor
        get() = engine.dspProcessor
    ...
    /**
     * Calculates stereo VU meter peak levels for visual presentation in Studio VU meters.
     */
