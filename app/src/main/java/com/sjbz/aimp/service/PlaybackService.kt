...
class PlaybackService : MediaSessionService() {
    ...
    fun setEmulationAmount(amount: Float) {
        ...
        dspProcessor.setEmulationAmount(clamped)
        prefs.edit().putFloat(KEY_EMU_AMOUNT, clamped).apply()
    }
    ...
    fun savePlaybackSession() {
        try {
            getSharedPreferences("sjbz_dsp_pro", Context.MODE_PRIVATE).edit().apply {
                putLong("last_track_id", getCurrentTrack()?.id?: -1L)
    ...
    fun restorePlaybackSession(): Pair<Int, Long> {
        val p = getSharedPreferences("sjbz_dsp_pro", Context.MODE_PRIVATE)
        return Pair(p.getInt("last_track_index", 0), p.getLong("last_position_ms", 0L))
    }
